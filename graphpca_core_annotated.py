...
__all__ = [
    ...
    "check_block_invariance", "pc_sign_flips", "align_pc_signs",
    "align_result_signs",
...
class AffinityConnectivity:
    ...
    orders of magnitude (1.57e-1 / 1.24e-6 = 1.3e5), so a heat time chosen
    as t = tau / lambda2 would be about 130000x larger than at bridge
    length 1. Counting zero modes does not catch it, because that
...
#
# A principal component is defined only up to sign. `pc_sign_flips` compares
# score columns with a reference fit and returns $-1$ for a column whose
# weighted overlap is negative. `align_result_signs` applies that vector to
# both the basis and the scores of a result, so the pair stays consistent;
# `align_pc_signs` applies it to a bare score array only. Either way this
# changes only the orientation of the axis, not the represented principal
# direction.
...
def pc_sign_flips(scores, reference_scores, mu=None, n_components=None):
    """The +-1 vector that aligns each score column with a reference fit.

    Column j is flipped when its weighted overlap with the reference,
    sum_y mu_y R[y, j] S[y, j], is negative. Columns beyond the reference's
    width (or beyond `n_components`) are left at +1. `mu=None` means unit
    weights. This is the single place the alignment rule is defined; both
    `align_pc_signs` and `align_result_signs` apply the vector it returns.
    """
    S = np.asarray(scores, float)
    R = np.asarray(reference_scores, float)
    if S.ndim != 2 or R.ndim != 2 or S.shape[0] != R.shape[0]:
        raise ValueError("scores and reference_scores must be 2-D with the "
                         "same number of rows (one per vertex).")
    k = min(S.shape[1], R.shape[1])
    if n_components is not None:
        k = min(k, int(n_components))
    weight = np.ones(S.shape[0]) if mu is None else np.asarray(mu, float).ravel()
    if weight.shape != (S.shape[0],):
        raise ValueError("mu must have one weight per vertex.")
    flips = np.ones(S.shape[1])
    if k:
        overlap = np.einsum("i,ij,ij->j", weight, R[:, :k], S[:, :k])
        flips[:k] = np.where(overlap < 0, -1.0, 1.0)
    return flips


def align_pc_signs(scores, reference_scores, mu=None, n_components=None):
    """Flip SCORE columns to agree in sign with a reference fit.
    ...
    This returns aligned scores ONLY. A PC is the pair (basis column, score
    column), and flipping one without the other leaves them inconsistent: the
    projection identity alpha_j(y) = gtilde_y.T C v_j no longer holds for the
    flipped columns. Note that `check_graphpca` does NOT detect this -- its
    three identities are each invariant under flipping one column -- so the
    mismatch surfaces only when basis and scores are used together, e.g. in a
    reconstruction or a plot of a PC field next to its scores. Use this when
    you have bare score arrays and nothing else. When you have a
    `GraphPCAResult`, use `align_result_signs`, which flips both together.
    """
    flips = pc_sign_flips(scores, reference_scores, mu, n_components)
    return np.asarray(scores, float) * flips[None, :]


def align_result_signs(result, reference_scores, mu=None, n_components=None):
    """Flip the basis AND score columns of a GraphPCAResult, consistently.

    `mu=None` uses `result.mu`, the measure the fit was made with, so the
    overlap is the same mu-weighted inner product `fit_graphpca` uses for its
    `sign_reference`. The result is modified in place and also returned; the
    sign vector applied is recorded in `result.diagnostics["sign_flips"]`.
    """
    weight = result.mu if mu is None else mu
    flips = pc_sign_flips(result.scores, reference_scores, weight, n_components)
    result.scores = result.scores * flips[None, :]
    result.basis = result.basis * flips[None, :]
    result.diagnostics["sign_flips"] = flips
    return result
...
def fit_graphpca(graph, potential_function="heat_kernel", t_values=(1.0,),
    ...
    for t in t_values:
        ...
        if sign_reference is not None and t in sign_reference:
            ...
            align_result_signs(result, reference, mu=fit.mu)
