...
You have a graph: n vertices, m edges, an incidence operator K of shape
...
Everything below is identical either way. GraphPCA is seven lines:
    ...
    U    = sqrt(ell) * (g_y - gbar) * sqrt(mu_y)       an (m, n) matrix
    U_left, s, _ = svd(U)                     # one SVD
    lambda_j = s_j ** 2                       # eigenvalue of PC j
    v_j      = U_left[:, j] / sqrt(ell)       # PC j, an edge field
...
do not depend on the image measure, so compute them once per diffusion time::
    ...
    result = graphpca(
        flows=flows_t, ell=ell, mu=image_mu,
        k_keep=20, solver="randomized", random_state=7,
    )
...
WHY sqrt(ell) AND sqrt(mu).  The edge space carries the metric
C = diag(ell), so lengths are measured with <a,b>_C = sum_e ell_e a_e b_e.
Multiplying by sqrt(ell) turns that into an ordinary dot product, which is
what an SVD understands; dividing U_left by sqrt(ell) at the end converts
back. The sqrt(mu) does the same job for the vertex weights.

WHY THE SVD AND NOT A SQUARED MATRIX.  The classical (n, n) Gram route
...
measured: relative error growing from ~1e-15 in the leading eigenvalue to
~1e-1 in the trailing ones once the spectrum spans 14 decades. The SVD of U
never forms either product: same answer, V.T C V = I by construction, and
full relative accuracy down the spectrum. Wall-clock differences between the
routes are modest either way at these sizes; accuracy, not speed, is why the
SVD is the default and "direct" is kept only as an independent check.

PC SIGNS.  A principal component is defined only up to sign: v and -v are the
same direction. By default this file pins a deterministic sign (largest-
magnitude entry of the left singular vector made positive), which REDUCES
arbitrary flips across runs and diffusion times but cannot eliminate them:
when two entries compete for largest magnitude, one ulp decides the sign, and
inside a degenerate block the axes rotate freely so no sign rule applies at
all. For guaranteed continuity across a t sweep, align against a reference
with `align_pc_signs` or pass `sign_reference` to `fit_graphpca`. None of
...
which is sqrt(PC1^2 + PC2^2) for a pair. `check_block_invariance`
demonstrates the invariance: it rotates the scores inside the block at random
and shows the magnitude stays put while the axes move. (That is a property of
the Euclidean norm, so it holds for ANY column group -- it certifies that the
magnitude is safe to interpret, not that the block is degenerate. Whether a
block IS degenerate is decided by the eigenvalue ratios in
`degenerate_blocks`.)
"""
...
def laplacian_eigh(laplacian, zero_tol=1e-12):
    ...
    # GraphPCA 0.8.0 clips eigensolver round-off here, and so do we -- but
    # only round-off. A negative eigenvalue LARGE relative to the spectrum is
    # not round-off: it means the input was not a Laplacian at all. The usual
    # causes are passing the adjacency W where L was meant, passing -L, or a
    # weight matrix with genuinely signed entries. Clipping those to zero
    # produces a plausible-looking spectrum from a meaningless input, and
    # every check downstream -- including check_graphpca -- then passes.
    #
...
# =====================================================================
# PART 1b -- from a weighted graph to (K, ell, L)
#
# These four steps are everything `graphpca` the package does BEFORE the
# PCA starts. Having them here means a notebook needs one file, not a file
# plus a package. Each is a few lines; the orderings are the only subtle
# part, so they are fixed once in `prepare_graph` and never re-derived.
# =====================================================================
@dataclass
class GraphSetup:
...
def graphpca(D=None, K=None, ell=None, mu=None, t=None, k_keep=40,
    ...
    """GraphPCA at one diffusion time. The module docstring has the maths.
...
# =====================================================================
# PART 2b -- the whole pipeline in one call
#
# `prepare_graph` + a potential + `graphpca` at each t. This is the
# `graphpca` package's `fit_graphpca` reproduced, so a notebook that used
# the package can drop it and keep every field name it was reading.
# =====================================================================
@dataclass
class GraphPCAFit:
...
# =====================================================================
# PART 3 -- degenerate blocks (averaging PCs)
# =====================================================================
...
# =====================================================================
# PART 5 -- optional: agreement with the graphpca package
# =====================================================================
def check_against_package(D_dict, K, ell, mu, t_values, k_keep=20,
                          eig_rtol=1e-10, atol=1e-8, verbose=True):
    """Compare this module with GraphPCA 0.8.0. Raises if they differ.

    The bases are compared through their C-orthogonal PROJECTORS, not column
    by column: inside a degenerate block the individual axes are arbitrary, so
    a column-wise test would report a disagreement that is not really there.
    """
    from graphpca import graphpca_covariance_gram, projection_coeffs_edge_evecs

    evals, evecs, gbar, totals = graphpca_covariance_gram(
        D_dict, K, ell, mu, t_values=list(t_values), k_keep=k_keep,
        eig_rtol=eig_rtol)
    coeffs = projection_coeffs_edge_evecs(D_dict, evecs, K, list(t_values), mu,
                                          edge_weight=ell)
    C = np.asarray(ell, float)[:, None]

    rows = []
    for t in t_values:
        mine = graphpca(D_dict[t], K, ell=ell, mu=mu, t=t, k_keep=k_keep,
                        eig_rtol=eig_rtol, warn=False)
        theirs = np.asarray(evals[t], float)[::-1]          # -> descending
        k = min(len(theirs), mine.n_components,
                np.asarray(coeffs[t]).shape[1])
        Vp = np.asarray(evecs[t], float)[:, ::-1][:, :k]
        Vo = mine.basis[:, :k]
        row = {
            "t": t, "k": k,
            "d_eigenvalues": float(np.max(np.abs(
                theirs[:k] - mine.eigenvalues[:k])) / max(theirs[0], 1e-300)),
            "d_projector": float(np.max(np.abs(
                Vp @ (C * Vp).T - Vo @ (C * Vo).T))),
            "d_scores_abs": float(np.max(np.abs(
                np.abs(np.asarray(coeffs[t], float)[:, :k])
                - np.abs(mine.scores[:, :k])))
                / max(np.abs(mine.scores[:, :k]).max(), 1e-300)),
            "d_total_var": float(abs(totals[t] - mine.total_variance)
                                 / max(totals[t], 1e-300)),
            "d_mean_flow": float(np.max(np.abs(
                np.asarray(gbar[t], float) - mine.mean_flow))
                / max(np.abs(mine.mean_flow).max(), 1e-300)),
        }
        rows.append(row)
        if verbose:
            print(f"  t={float(t):<8g} d_eig {row['d_eigenvalues']:.2e}   "
                  f"d_projector {row['d_projector']:.2e}   "
                  f"d_scores {row['d_scores_abs']:.2e}   "
                  f"d_var {row['d_total_var']:.2e}")
        for key, value in row.items():
            if key.startswith("d_") and value >= atol:
                raise AssertionError(f"{key} = {value:.3e} at t={t}")
    if verbose:
        print("PASS: graphpca_core reproduces the package.")
    return rows
